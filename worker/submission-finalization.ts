export const FINALIZE_SUBMISSION_SQL = "UPDATE submissions SET state=?, verdict=?, score=?, fully_passed_cases=?, deterministic_cost=?, peak_memory_bytes=?, effective_attempt=?, updated_at=?, completed_at=? WHERE id=? AND state NOT IN ('completed','compile-error','judge-error','infrastructure-error','cancelled') AND ?=(SELECT MAX(attempt) FROM submission_attempts WHERE submission_id=?) AND EXISTS (SELECT 1 FROM submission_attempts WHERE submission_id=? AND attempt=? AND state='running' AND token_hash=?)";

export const FINALIZE_SUBMISSION_ATTEMPT_SQL = "UPDATE submission_attempts SET state='succeeded', finished_at=? WHERE submission_id=? AND attempt=? AND state='running' AND token_hash=? AND ((? IS NULL AND audit_r2_key IS NULL) OR audit_r2_key=?) AND EXISTS (SELECT 1 FROM submissions WHERE id=? AND effective_attempt=? AND state=?)";
...
export function finalizedSubmissionAttemptMatches(
  ...
  expected: {
    ...
    readonly auditR2Key: string | null;
  },
): boolean {
  return record?.state === expected.state
    ...
    && record.audit_r2_key === expected.auditR2Key;
}
