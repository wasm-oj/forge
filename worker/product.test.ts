...
import { acknowledgeRepositoryPushNotice, getContest, listProblems, listRepositoryPushNotices, managedProblemProjection, problemLeaderboard, publicProfile } from "./product";
...
function environment(identity: { readonly visibility: "public" | "private"; readonly status: "active" | "suspended" }, mode: "official-practice" | "contest" = "official-practice"): ForgeWorkerEnv {
  const prepare = vi.fn((sql: string) => ({
    bind: () => ({
      first: async () => {
        if (sql.includes("FROM managed_problem_versions JOIN managed_snapshots")) return mode === "official-practice" ? { id: PROBLEM_ID } : null;
  ...
  const submissionsPrepare = vi.fn(() => ({
    bind: () => ({
      all: async () => ({ results: [{
        user_id: USER_ID,
        score: 100,
  ...
  return {
    CORE_DB: { prepare } as unknown as D1Database,
    SUBMISSIONS_DB: { prepare: submissionsPrepare } as unknown as D1Database,
...
describe("public problem catalog", () => {
  it("pins the configured official collection first and permits anonymous caching", async () => {
    ...
    const prepare = vi.fn(() => ({ bind: () => ({ all: async () => ({ results: [{
      ...
      id: PROBLEM_ID, problem_slug: "two-sum", problem_number: 1, title_json: title,
    ...
    expect(await response.json()).toMatchObject({ collections: [{ official: true, repository: { name: "official-problems" }, problems: [{ id: PROBLEM_ID, solved: false, difficulty: "easy" }] }] });
  });
});
...
async function noticeEnvironment(acknowledgedAt: string | null = null): Promise<ForgeWorkerEnv> {
  ...
  return { CORE_DB: { prepare } as unknown as D1Database, PUBLIC_ORIGIN: "https://forge.test" } as unknown as ForgeWorkerEnv;
}
...
describe("contest start-time confidentiality", () => {
  ...
  it("rejects a pre-start public projection before either R2 bucket is read", async () => {
    const get = vi.fn();
    ...
    await expect(managedProblemProjection(new Request(`https://forge.test/api/problems/${PROBLEM_ID}?contestId=55555555-5555-4555-8555-555555555555`), {
      ...
      JUDGE_MIRROR_BUCKET: { get } as unknown as R2Bucket,
