...
type WasmOjTurnstileAction = "official-submit" | "organizer-application";

export const TURNSTILE_CHALLENGE_PATH = "/turnstile/challenge";

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
const ACTIONS = new Set<WasmOjTurnstileAction>(["official-submit", "organizer-application"]);
...
function challengeDocument(appOrigin: string, siteKey: string, action: WasmOjTurnstileAction, requestNonce: string): Response {
...
<head>
  ...
  <title>WASM-OJ verification</title>
  <style nonce="${cspNonce}">
    ...
    #wasm-oj-turnstile { display: flex; justify-content: center; min-height: 70px; }
  </style>
  <script nonce="${cspNonce}">
    ...
    window.wasmOjTurnstileDone = (token) => {
      send({ type: "wasm-oj-platform/turnstile/v1", action: configuration.action, nonce: configuration.requestNonce, token });
      window.setTimeout(() => window.close(), 150);
    };
    window.wasmOjTurnstileFailed = () => send({ type: "wasm-oj-platform/turnstile-error/v1", action: configuration.action, nonce: configuration.requestNonce, code: "turnstile-failed" });
    window.wasmOjTurnstileExpired = () => send({ type: "wasm-oj-platform/turnstile-error/v1", action: configuration.action, nonce: configuration.requestNonce, code: "turnstile-expired" });
  </script>
  <script nonce="${cspNonce}" src="https://challenges.cloudflare.com/turnstile/v0/api.js" defer></script>
</head>
<body>
  <main>
    <h1>Verify this WASM-OJ action</h1>
    <p>This window sends only a one-time challenge token back to WASM-OJ.</p>
    <div id="wasm-oj-turnstile" class="cf-turnstile" data-sitekey="${htmlEscape(siteKey)}" data-action="${action}" data-callback="wasmOjTurnstileDone" data-error-callback="wasmOjTurnstileFailed" data-expired-callback="wasmOjTurnstileExpired"></div>
  </main>
</body>
</html>
`;
...
export function turnstileChallengeResponse(request: Request, env: WasmOjWorkerEnv): Response {
  ...
  const action = url.searchParams.get("action") as WasmOjTurnstileAction | null;
