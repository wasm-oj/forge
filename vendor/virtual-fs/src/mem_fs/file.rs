...
impl File {
    pub fn write(&mut self, buf: &[u8], cursor: &mut u64) -> io::Result<usize> {
        ...
        if end > self.buffer.len() {
            // Grow the entire sparse range transactionally before modifying any
            // visible bytes. A quota failure therefore cannot leave a partial gap.
