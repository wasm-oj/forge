...
`ForgeDependencyManager` is the host-neutral dependency contract for Cargo crates, npm packages, PyPI distributions, Go modules, and C/C++ libraries. Ecosystem resolvers receive only their tagged native manifests, lockfiles, and sources; Forge emits one canonical graph lock bound to the complete manifest SHA-256, stores every package by payload digest, and supports integrity-checked browser IndexedDB, server filesystem, and offline bundle transport. In offline mode the manager never calls a resolver: it requires a matching previous lock and verifies every cached payload.
...
- C/C++ verifies the decompressed Clang WebC, WASI P1 sysroot, and single pinned cc1/`wasm-ld` manifest by SHA-256 before either target profile is built. Within a bounded Worker generation, unchanged translation units reuse dependency-validated objects; the default same-origin toolchain responses remain in Cache Storage across generations. The current C++ package does not contain an admitted libc++ PCH, so header-heavy C++ builds remain outside the browser compile SLO.
