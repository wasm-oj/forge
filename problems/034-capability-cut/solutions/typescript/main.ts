...
type E = { to: number; rev: number; cap: bigint };
...
let n = nextInt(),
  ...
  sn = nextInt(),
  tn = nextInt();
const c: bigint[] = Array.from({ length: n }, () => nextBigInt()),
  en: number[] = Array.from({ length: sn }, () => nextInt() - 1),
  dn: number[] = Array.from({ length: tn }, () => nextInt() - 1),
  V = 2 * n + 2,
  S = 2 * n,
  T = S + 1,
  g: E[][] = Array.from({ length: V }, () => []),
  add = (u: number, v: number, z: bigint): void => {
    g[u].push({ to: v, rev: g[v].length, cap: z });
    g[v].push({ to: u, rev: g[u].length - 1, cap: 0n });
  },
  inf = c.reduce((x, y) => x + y, 1n);
...
let flow = 0n;
for (;;) {
  const level = new Int32Array(V).fill(-1), q: number[] = [S];
  level[S] = 0;
  ...
  const dfs = (u: number, f: bigint): bigint => {
    if (u === T) return f;
