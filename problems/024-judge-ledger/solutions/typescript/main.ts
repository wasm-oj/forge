...
while (b < n) b *= 2;
const bad: number[] = Array(n + 2).fill(0),
  nb: number[] = Array(n + 2).fill(n + 1),
  u: number[][] = Array.from({ length: 4 }, () => Array(n + 1).fill(0)),
  s: bigint[][] = Array.from({ length: 2 }, () => Array(n + 1).fill(0n)),
  tm: bigint[] = Array(2 * b).fill(0n),
  tv: bigint[] = Array(2 * b).fill(0n);
for (let i = 1; i <= n; i++) {
  bad[i] = +x[p++];
...
function rmq(t: bigint[], l: number, r: number) {
  l += b - 1;
  r += b - 1;
  let z = 0n;
  while (l <= r) {
