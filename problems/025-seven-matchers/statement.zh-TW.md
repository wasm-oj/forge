...
WASM OJ 的答案驗證不能假設「正確」永遠代表輸出文字逐 byte 相同。有些題目在意完整字串，有些允許忽略集合順序，有些要保留重複次數，還有些需要比較數值誤差或一組輸出檔案。

為了讓每題明確選擇自己的判定語意，系統提供七種 matcher。每個 query 都包含 expected 與 actual 兩個 token 陣列，並指定其中一種 matcher。`n`、`m` 可為 0；對應的資料行此時是空行。
