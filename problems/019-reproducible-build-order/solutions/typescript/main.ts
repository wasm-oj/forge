...
let p = 0, n = Number(a[p++]), m = Number(a[p++]);
const names: string[] = Array.from({ length: n }, () => a[p++]),
  nameOrder: number[] = Array.from({ length: n }, (_, i) => i).sort((x, y) =>
    names[x] < names[y] ? -1 : names[x] > names[y] ? 1 : 0
  ),
  g: number[][] = Array.from({ length: n }, () => []),
...
if (bad) std.out.puts(`INVALID DANGLING ${bad}\n`);
  ...
  const h: number[] = [],
    less = (x: number, y: number): boolean => names[x] < names[y],
    push = (x: number): void => {
      let i = h.length;
      h.push(x);
      while (i) {
