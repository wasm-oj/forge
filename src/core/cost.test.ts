...
const raw = {
  cost: JAVA_EMPTY_PROGRAM_BASELINE_COST.release,
  costModel: "weighted",
  operations: {},
  memoryBytes: 0,
  logicalTimeNs: 0,
...
describe("cost baselines", () => {
  it("ships the measured Java empty-program baseline for both optimization profiles", () => {
    ...
    expect(registry.baseline(javaReleaseProfile)).toBe(JAVA_EMPTY_PROGRAM_BASELINE_COST.release);
    expect(registry.baseline(javaDebugProfile)).toBe(JAVA_EMPTY_PROGRAM_BASELINE_COST.debug);
  });
  ...
  it("normalizes the measured empty Java program to zero net cost", () => {
    const metrics = normalizeExecutionMetrics(raw, {
      ...
      baselineCost: JAVA_EMPTY_PROGRAM_BASELINE_COST.release,
      netInstructionBudget: 10000,
      rawInstructionBudget: JAVA_EMPTY_PROGRAM_BASELINE_COST.release + 10000,
    });

    expect(metrics).toMatchObject({
      cost: 0,
      rawCost: JAVA_EMPTY_PROGRAM_BASELINE_COST.release,
      baselineCost: JAVA_EMPTY_PROGRAM_BASELINE_COST.release,
    });
  });
});
