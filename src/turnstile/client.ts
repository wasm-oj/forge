export type ForgeTurnstileAction = "official-submit" | "organizer-application";

interface ForgeTurnstileSuccessMessage {
  readonly type: "forge-turnstile-v1";
  readonly action: ForgeTurnstileAction;
  readonly nonce: string;
  readonly token: string;
}

interface ForgeTurnstileFailureMessage {
  readonly type: "forge-turnstile-error-v1";
  readonly action: ForgeTurnstileAction;
  readonly nonce: string;
...
export function parseForgeTurnstileMessage(
  value: unknown,
  expectedAction: ForgeTurnstileAction,
  expectedNonce: string,
): ForgeTurnstileSuccessMessage | ForgeTurnstileFailureMessage | undefined {
  const message = record(value);
  if (!message || message.action !== expectedAction || message.nonce !== expectedNonce) return undefined;
  if (
    message.type === "forge-turnstile-v1"
    && typeof message.token === "string"
    && message.token.length > 0
    && new TextEncoder().encode(message.token).byteLength <= TOKEN_MAXIMUM_BYTES
    && !/[\u0000-\u0020\u007f]/.test(message.token)
  ) {
    return message as unknown as ForgeTurnstileSuccessMessage;
  }
  if (
    message.type === "forge-turnstile-error-v1"
    && (message.code === "turnstile-failed" || message.code === "turnstile-expired")
  ) {
    return message as unknown as ForgeTurnstileFailureMessage;
  }
  return undefined;
}

export async function requestForgeTurnstileToken(
  action: ForgeTurnstileAction,
): Promise<string> {
  ...
  overlay.setAttribute("aria-label", "Forge Turnstile verification");
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const onMessage = (event: MessageEvent): void => {
      ...
      const message = parseForgeTurnstileMessage(event.data, action, nonce);
      if (!message) return;
      if (message.type === "forge-turnstile-v1") finish({ token: message.token });
