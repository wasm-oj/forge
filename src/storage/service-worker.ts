export interface ToolchainCacheRegistrationOptions {
  ...
  /** Service-worker scope covering the configured WASM-OJ toolchain asset base URL. */
  scope?: string;
...
export async function registerToolchainCache(
  ...
  if (typeof scope !== "string" || !scope) {
    throw new TypeError("WASM-OJ toolchain cache service-worker scope must be a non-empty string.");
  }
...
async function configureChunkManifest(
  ...
  if (manifestUrl !== undefined) {
    ...
    if (url.origin !== location.origin || url.search || url.hash
      || url.pathname !== "/toolchains/wasm-oj-sites-chunks.json") {
      throw new Error("WASM-OJ toolchain chunk manifest must use the canonical same-origin deployment path.");
    }
    manifestPath = url.pathname;
  }
  const channel = new MessageChannel();
  const response = new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error("WASM-OJ toolchain chunk configuration timed out.")), 5_000);
    channel.port1.onmessage = (event: MessageEvent<unknown>) => {
      clearTimeout(timeout);
      channel.port1.close();
      if (event.data && typeof event.data === "object" && "ok" in event.data && event.data.ok === true) resolve();
      else reject(new Error("WASM-OJ toolchain cache service worker rejected its chunk manifest configuration."));
    };
  });
  worker.postMessage(
...
function waitForActiveWorker(registration: ServiceWorkerRegistration): Promise<ServiceWorker> {
  ...
  if (!worker) {
    return Promise.reject(new Error("WASM-OJ toolchain cache registration has no service-worker instance."));
  }
  ...
  return new Promise<ServiceWorker>((resolve, reject) => {
    const onStateChange = () => {
      if (worker.state === "activated") {
        ...
        worker.removeEventListener("statechange", onStateChange);
        reject(new Error("WASM-OJ toolchain cache service worker became redundant before activation."));
      }
    };
