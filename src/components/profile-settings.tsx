...
import type { Profile } from "./education-model";
import { Drawer } from "./drawer";
import { IconButton } from "./icon-button";
import { forgeJson, forgeMutation } from "./online-api";
import { usePageTitle } from "./page-title";
import { RemoteStateView, type RemoteState } from "./remote-state";
...
const TEXT = {
  en: {
    ...
    dangerTitle: "Delete account", dangerDetail: "Permanently remove your Forge account and anonymize server submissions.", delete: "Delete account",
    ...
    deleted: "Your Forge account was deleted and you have been signed out.", queued: "Account deletion started. Access has been revoked and remaining archives will be removed shortly.",
  },
  "zh-TW": {
    ...
    dangerTitle: "刪除帳號", dangerDetail: "永久刪除 Forge 帳號，並匿名化 Server 提交紀錄。", delete: "刪除帳號",
    ...
    deleted: "Forge 帳號已刪除，並已登出。", queued: "帳號刪除已開始，存取權已撤銷；其餘封存資料會隨後移除。",
  },
} as const;
...
export function ProfileSettings() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      const value = await forgeJson<{ profile: Profile }>("/api/profile");
  ...
  async function save(event: FormEvent<HTMLFormElement>, profile: Profile) {
    ...
    setSaveMessage("");
    try {
      await forgeMutation("/api/profile", {
  ...
  async function deleteAccount(profile: Profile) {
    ...
    setDeleteError("");
    try {
      const result = await forgeMutation<AccountErasureResponse>("/api/account", {}, "DELETE");
  ...
  if (erasureMessage) return <main className="product-page narrow-page" id="main-content"><section className="account-erasure-complete" role="status"><UserRound aria-hidden="true" size={28} /><h1>{erasureMessage}</h1><Link className="primary-action" href="/">Forge</Link></section></main>;
