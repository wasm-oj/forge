...
export function LearningDashboard() {
  ...
  const loadContests = useCallback(async () => {
    ...
    try {
      const value = await forgeJson<{ contests: readonly ContestSummary[] }>("/api/contests?limit=6");
  ...
  const loadSubmissions = useCallback(async () => {
    ...
    try {
      const value = await forgeJson<{ submissions: readonly SubmissionSummary[] }>("/api/submissions?limit=5");
