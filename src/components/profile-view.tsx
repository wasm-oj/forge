...
export function ProfileView({ login }: { readonly login: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await forgeJson<{ profile: PublicProfile }>(`/api/profiles/${encodeURIComponent(login)}`);
  ...
  return <main className="product-page profile-shell" id="main-content">
    ...
    <RemoteStateView state={state} loadingLabel={text.loading} retryLabel={text.retry} empty={null} isEmpty={() => false}>{(current) => <>
      ...
      <section className="organizer-product-section"><h2>{text.solves}</h2><div className="profile-solve-list">{current.verifiedSolves.map((solve) => <Link href={`/problems/${encodeURIComponent(solve.managedProblemVersionId)}`} key={solve.managedProblemVersionId}><div><strong>{solve.title[locale] ?? solve.title.en ?? solve.title["zh-TW"] ?? solve.problemSlug}</strong><span>{new Date(solve.solvedAt).toLocaleDateString(locale)}</span></div><b>{solve.score}</b></Link>)}</div>{current.verifiedSolves.length === 0 && <p className="product-empty">{text.empty}</p>}</section>
    </>}</RemoteStateView>
  </main>;
}
