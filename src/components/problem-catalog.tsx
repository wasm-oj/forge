...
import type { CatalogProblem } from "./education-model";

function hasCurrentLocalSamplesPassed(records: ReadonlyMap<string, LocalSamplesPassedRecord>, problem: CatalogProblem): boolean {
  return hasMatchingLocalSamplesPassed(records, problem.id, problem.bundleDigest);
}
...
export function ProblemCatalog() {
  ...
  const visibleCollections = collections.filter((collection) => collectionId === "all" || collection.snapshotId === collectionId).map((collection) => ({
    collection,
  ...
  return <main className="product-page" id="main-content">
    ...
    <div className="catalog-toolbar">
      ...
      <FilterField label={labels.collection}><select aria-label={labels.collection} value={collectionId} onChange={(event) => setCollectionId(event.target.value)}><option value="all">{labels.allCollections}</option>{collections.map((collection) => <option key={collection.snapshotId} value={collection.snapshotId}>{collection.official ? `Official · ${collection.repository.name}` : `${collection.repository.owner}/${collection.repository.name}`}</option>)}</select></FilterField>
