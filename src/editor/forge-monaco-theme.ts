...
export type ForgeMonacoTheme = "forge-nord-light" | "forge-nord-dark";
...
export const FORGE_MONACO_THEMES = Object.freeze({
  "forge-nord-light": {
    base: "vs",
    inherit: true,
    rules: SHARED_RULES,
...
} satisfies Readonly<Record<ForgeMonacoTheme, Monaco.editor.IStandaloneThemeData>>);

export function forgeMonacoTheme(productTheme: "light" | "dark"): ForgeMonacoTheme {
  return productTheme === "dark" ? "forge-nord-dark" : "forge-nord-light";
}

export function registerForgeMonacoThemes(monaco: typeof Monaco): void {
  for (const [name, theme] of Object.entries(FORGE_MONACO_THEMES)) {
    monaco.editor.defineTheme(name as ForgeMonacoTheme, theme);
  }
}
