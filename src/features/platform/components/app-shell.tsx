...
const THEME_STORAGE_KEY = "wasm-oj:product-theme:v1";
const SIDEBAR_STORAGE_KEY = "wasm-oj:sidebar-collapsed:v1";
...
export function AppShell({ children }: { readonly children: ReactNode }) {
  ...
  const refreshSession = useCallback(async () => {
    ...
    try {
      setSession(await wasmOjJson<ProductSession>("/api/auth/session"));
  ...
  function sidebar(close: () => void, collapsed: boolean, mobile: boolean) {
    return <>
      <div className="app-brand">
        <Link href="/" aria-label="WASM-OJ" onClick={close}><span className="app-brand-mark"><Command aria-hidden="true" size={17} /></span><strong>WASM-OJ</strong></Link>
  ...
  if (!preferencesReady || sessionStatus === "loading") {
    return <div className="app-shell app-shell-hydrating" aria-label="WASM-OJ" aria-busy="true" role="status"><aside aria-hidden="true" /><div aria-hidden="true" /></div>;
  }
  ...
  return <ProductContext.Provider value={context}>
    <div className={sidebarCollapsed ? "app-shell is-sidebar-collapsed" : "app-shell"}>
      ...
      <button ref={menuButtonRef} className="app-mobile-menu" type="button" aria-label={text.openMenu} aria-expanded={mobileOpen} aria-controls="mobile-navigation" onClick={() => setMobileOpen(true)} data-drawer-background><Menu aria-hidden="true" size={20} /><span>WASM-OJ</span></button>
