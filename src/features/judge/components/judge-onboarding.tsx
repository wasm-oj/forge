...
} from "../model/judge-onboarding-storage";
  ...
  return (
    <div className="onboarding-step-body onboarding-welcome">
      ...
      <p className="onboarding-lead">
        {zh
          ? "Online Judge（OJ）會用多組測資執行你的程式，再依輸出與資源使用量給出判定。WASM-OJ 保留熟悉的解題流程，但把編譯器、執行環境與 Judge 全部搬進你的瀏覽器。"
          : "An Online Judge (OJ) runs your program against multiple cases and evaluates its output and resource use. WASM-OJ keeps that familiar workflow while moving the compiler, runtime, and judge into your browser."}
      </p>
...
function BrowserJudgeStep({ zh }: { zh: boolean }) {
  return (
    <div className="onboarding-step-body">
      <p className="onboarding-eyebrow">{zh ? "WASM-OJ 的核心差異" : "THE WASM-OJ DIFFERENCE"}</p>
...
function ComparisonStep({ zh }: { zh: boolean }) {
  ...
  return (
    <div className="onboarding-step-body">
      ...
      <h2 tabIndex={-1}>{zh ? "WASM-OJ 與一般伺服器 Judge 的差異" : "WASM-OJ versus a typical server-side judge"}</h2>
      ...
      <div className="onboarding-comparison" role="table" aria-label={zh ? "Judge 差異比較" : "Judge comparison"}>
        <div className="onboarding-comparison-head" role="row"><span role="columnheader">{zh ? "面向" : "Aspect"}</span><span role="columnheader"><Cloud size={14} />{zh ? "一般伺服器 OJ" : "Typical server OJ"}</span><span role="columnheader"><Laptop size={14} />WASM-OJ</span></div>
        {rows.map(([aspect, traditional, wasmOj]) => <div role="row" key={aspect}><strong role="cell">{aspect}</strong><span role="cell">{traditional}</span><span role="cell">{wasmOj}</span></div>)}
      </div>
    ...
    <div className="onboarding-backdrop">
      ...
      >
        <header className="onboarding-header">
          <div><span className="brand-mark"><Terminal size={16} /></span><div><strong id="judge-onboarding-title">WASM-OJ Judge</strong><span>{zh ? "新手導覽" : "Getting started"}</span></div></div>
