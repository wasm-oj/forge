...
export const JUDGE_UI_LOCALE_STORAGE_KEY = `${WASM_OJ_CONTRACT_ID}:judge-ui-locale:v1`;
...
const ZH_TW = {
  ...
  official: {
    ...
    contestSamplesOnly: "公開題目內容只包含範例測資；可在瀏覽器建置、執行與自行測試，完整 verdict 必須使用正式提交。",
  ...
  catalog: {
    ...
    verifiedManaged: "已驗證的 exact-commit 內容",
  ...
  settings: {
    ...
    managedContentDescription: "此工作區先取得 D1 的版本指標，再讀取 GitHub exact commit 的公開題目內容；正式提交身分不會由 GitHub 座標推測。",
    ...
    rustToolchainNote: (version: string) => `使用來源可追溯的 rustc ${version} WebC 與相符的標準函式庫，在 Wasmer 內直接產生 WASI P1；Cargo 可使用 WASM-OJ 統一 lock/cache API，但目前編輯器尚未把解析後的 crate 掛載進 rustc build。`,
    goToolchainTitle: "標準 Go／wasip1",
    goToolchainNote: (version: string) => `使用標準 Go ${version} 編譯器、linker 與相符的 349-package 標準函式庫，全程在 Wasmer 內產生 WASI P1；Go modules 可使用 WASM-OJ 統一 lock/cache API。`,
    executionEyebrow: "執行",
    executionTitle: "執行限制",
    ...
    browserQuota: "瀏覽器配額",
    clearCache: "清除快取",
    draftPersistence: "程式碼草稿",
    draftSaved: "最新修改已安全寫入此瀏覽器。",
    draftSaving: "正在將最新修改寫入此瀏覽器。",
    draftPending: "最新修改仍在記憶體中，將自動儲存。",
    draftError: (detail: string) => `尚未儲存；最新草稿仍保留在此分頁。${detail}`,
    retryDraftSave: "重試儲存",
    draftRecovery: "草稿人工復原",
    draftRecoveryDescription: "只匯出或匯入檔案路徑、語言與原始碼；不包含執行輸入、環境變數或帳號資料。",
    exportDraft: "匯出原始碼",
    importDraft: "匯入原始碼",
    draftExported: "已下載經驗證的原始碼草稿備份。",
    draftImported: "原始碼草稿已驗證、套用並寫入此瀏覽器。",
    draftRecoveryError: (detail: string) => `草稿復原失敗：${detail}`,
    persistentStorage: "持久儲存",
    persistentStorageGranted: "瀏覽器已允許持久儲存，可降低自動清除本機資料的風險。",
    persistentStorageBestEffort: "瀏覽器未允許持久儲存；空間不足時可能清除本機資料。",
    persistentStorageError: "無法確認持久儲存權限；本機資料目前仍採 best-effort 儲存。",
    ...
    toolchainNotes: {
      ...
      python: "封裝 Python 原始碼，由 CPython wasm32-wasip1 直譯器在執行時解析程式與匯入模組。",
  ...
  logs: {
    ...
    cancelled: "已取消操作並重啟 Compiler／Runner Workers",
...
const EN = {
  ...
  topbar: {
    ...
    stop: "Stop",
    build: "Build",
    runPublicSamples: "Run Public Samples",
    selfTest: "Self Test",
  ...
  official: {
    ...
    contestSamplesOnly: "Public problem content contains samples only. Build, run, and self-test in the browser; use Official Submit for a complete verdict.",
  ...
  catalog: {
    ...
    verifiedManaged: "verified exact-commit content",
  ...
  status: {
    ...
    draftPending: "Draft not yet saved",
  ...
  settings: {
    ...
    managedContentDescription: "This workspace reads a D1 version pointer, then public problem content from the exact GitHub commit. Formal submission identity is never inferred from GitHub coordinates.",
    ...
    rustToolchainNote: (version: string) => `Uses a provenance-traceable rustc ${version} WebC and matching standard library to produce WASI P1 directly inside Wasmer. Cargo can use WASM-OJ's unified lock/cache API, but the editor does not yet mount resolved crates into rustc builds.`,
    goToolchainTitle: "Standard Go / wasip1",
    goToolchainNote: (version: string) => `Uses the standard Go ${version} compiler, linker, and matching 349-package standard library to produce WASI P1 entirely inside Wasmer. Go modules can use WASM-OJ's unified lock/cache API.`,
    ...
    draftPersistence: "Source-code draft",
    draftSaved: "The latest changes are safely stored in this browser.",
    draftSaving: "Writing the latest changes to this browser.",
    draftPending: "The latest changes remain in memory and will be saved automatically.",
    draftError: (detail: string) => `Not saved; the latest draft remains in this tab. ${detail}`,
    retryDraftSave: "Retry save",
    draftRecovery: "Manual draft recovery",
    draftRecoveryDescription: "Export or import only file paths, languages, and source code; execution input, environment variables, and account data are excluded.",
    exportDraft: "Export sources",
    importDraft: "Import sources",
    draftExported: "Downloaded a validated source-only draft backup.",
    draftImported: "The source draft was validated, applied, and stored in this browser.",
    draftRecoveryError: (detail: string) => `Draft recovery failed: ${detail}`,
    persistentStorage: "Persistent storage",
    persistentStorageGranted: "The browser granted persistent storage, reducing the risk of automatic local-data eviction.",
    persistentStorageBestEffort: "The browser did not grant persistent storage; local data may be evicted when space is constrained.",
    persistentStorageError: "Persistent-storage permission could not be confirmed; local data remains best-effort.",
    ...
    toolchainNotes: {
      ...
      python: "Python source files parsed at execution by the source-built CPython wasm32-wasip1 interpreter and standard library.",
  ...
  logs: {
    ...
    cancelled: "Cancelled the operation and restarted the Compiler / Runner Workers",
