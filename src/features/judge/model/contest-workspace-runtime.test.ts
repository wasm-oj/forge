...
import type { ContestWorkspaceRuntime } from "./contest-workspace-runtime";
import { promptProgramBlockedReason } from "./contest-workspace-runtime";
...
describe("Prompt Program workspace admission state", () => {
  it("allows only an available, open, active prompt track", () => {
    expect(promptProgramBlockedReason(runtime)).toBeNull();
    expect(promptProgramBlockedReason({ ...runtime, promptCompilerAvailable: false })).toMatch(/unavailable/);
    expect(promptProgramBlockedReason({ ...runtime, paused: true })).toMatch(/paused/);
    expect(promptProgramBlockedReason({ ...runtime, availability: "closed" })).toMatch(/closed/);
    expect(promptProgramBlockedReason({ ...runtime, entrantState: "eliminated" })).toMatch(/eliminated/);
    expect(promptProgramBlockedReason({ ...runtime, attemptsRemaining: 0 })).toMatch(/remain/);
  });
});
