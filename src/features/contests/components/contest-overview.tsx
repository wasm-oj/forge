...
export function ContestOverview({ contestId }: { readonly contestId: string }) {
  ...
  const load = useCallback(async () => {
    const detail = await wasmOjJson<{ contest: ContestDetail; problems: ContestProblem[] }>(`/api/contests/${encodeURIComponent(contestId)}`);
    ...
    if (detail.contest.accessMode === "public" || detail.contest.joined || detail.contest.organizer) {
      const board = await wasmOjJson<{ frozen: boolean; entries: PublicLeaderboardEntry[] }>(`/api/contests/${encodeURIComponent(contestId)}/leaderboard`);
  ...
  const problemColumns = problems.map((problem) => ({ id: problem.problemId, label: String(problem.ordinal) }));
  ...
  return <main className="product-page" id="main-content">
    ...
    {contest && <>
      ...
      <section className="organizer-product-section"><h2>{zh ? "題目" : "Problems"}</h2><div className="contest-problem-list">{problems.map((problem) => <Link key={problem.problemId} href={managedProblemWorkspacePath({ contestId, problemId: problem.problemId })}><span>{problem.ordinal}</span><strong>{localized(problem.title, locale, problem.problemSlug)}</strong><span>{zh ? "開啟 →" : "Open →"}</span></Link>)}</div>{problems.length === 0 && <p className="product-empty">{contest.phase === "upcoming" ? (zh ? "題目將在競賽開始時公開。" : "Problems open when the contest starts.") : contest.accessMode === "invite" && !contest.joined && !contest.organizer ? (zh ? "加入後才能查看邀請競賽題目。" : "Join this invite-only contest to view its problems.") : (zh ? "目前沒有題目。" : "No problems are available.")}</p>}</section>
