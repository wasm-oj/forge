...
export function ContestList() {
  ...
  return <main className="product-page" id="main-content"><header className="product-page-header"><span className="product-eyebrow"><Trophy size={14} /> Compete</span><h1>{copy[locale].contests}</h1><p>{copy[locale].upcoming}</p></header>{error && <div className="product-error">{error}</div>}<div className="contest-catalog">{(["running", "upcoming", "ended"] as const).map((group) => { const grouped = contests.filter((contest) => contestCatalogGroup(contest.phase) === group); return <section key={group}><h2>{group}</h2><div className="contest-card-grid">{grouped.map((contest) => <Link className="contest-card" key={contest.id} href={`/contests/${contest.id}`}><div><span className={`contest-phase ${group}`}>{contest.phase}</span><span>{contest.officialTrack.kind === "prompt-program" ? "Prompt Program" : contest.accessMode}</span></div><h3>{contest.title}</h3><p>{contest.description}</p><footer><span><Clock3 size={14} />{formatDate(contestPrimaryWallTime(contest), locale)}</span><ChevronRight size={16} /></footer></Link>)}</div>{grouped.length === 0 && <p className="product-empty">No {group} contests.</p>}</section>; })}</div></main>;
}
