...
export function AdminOrganizerApplications() {
  ...
  const load = useCallback(async () => {
    try {
      const result = await wasmOjJson<{ applications: readonly Application[] }>(`/api/admin/organizer-applications?status=${status}`);
  ...
  useEffect(() => {
    let active = true;
    void wasmOjJson<{ applications: readonly Application[] }>(`/api/admin/organizer-applications?status=${status}`)
  ...
  async function confirmAction() {
    ...
    try {
      if (action.type === "revoke") {
        await wasmOjMutation(`/api/admin/organizers/${encodeURIComponent(action.application.user_id)}/revoke`, {});
        setMessage(`Organizer role removed for @${action.application.login}.`);
      } else {
        await wasmOjMutation(`/api/admin/organizer-applications/${encodeURIComponent(action.application.id)}/review`, { decision: action.type === "approve" ? "approved" : "rejected", ...(action.type === "reject" ? { reason } : {}) });
