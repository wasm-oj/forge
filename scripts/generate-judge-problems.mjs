...
const STARTER_ENTRY_BY_LANGUAGE = Object.freeze({
  c: "src/main.c",
  cpp: "src/main.cpp",
  rust: "src/main.rs",
  go: "src/main.go",
...
function builtInStarterSource(problem, language) {
  const heading = `Problem ${problem.number}: ${problem.title["zh-TW"]}`;
  switch (language) {
    case "c":
      return [
        "#include <stdio.h>",
    ...
    default:
      fail(`unsupported built-in starter language '${language}'`);
  }
}

function withBuiltInStarterTemplates(problem) {
  return {
    ...problem,
    starterTemplates: Object.fromEntries(LANGUAGES.map((language) => {
      const entry = STARTER_ENTRY_BY_LANGUAGE[language];
      return [language, { entry, files: { [entry]: builtInStarterSource(problem, language) } }];
    })),
  };
}
...
async function main() {
  ...
  const problems = applyLearningPath(learningPath, canonicalProblems).map(withBuiltInStarterTemplates);
